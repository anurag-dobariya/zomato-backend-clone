...
// creating controller for get restaurant images list
const getRestaurantImages = async (req, res) => {
  try {
    const getList = await restaurantService.getRestaurantImages(req, res);

    res.status(200).json({
      success: true,
      message: "Restaurant images list got successfully.",
      data: getList,
    });
  } catch (error) {
    res.status(400).json({
...
const getRestaurantById = async (req, res) => {
  try {
    const getRestaurant = await restaurantService.getRestaurantById(
...
const deleteRestaurant = async (req, res) => {
  try {
    ...
    const restaurantExists = await galleryService.getRestaurantById(
      restaurantId
    );
    if (!restaurantExists) {
      throw new Error("Restaurant not found!");
    ...
    const deletedRestaurant = await galleryService.deleteRestaurant(
      restaurantId
    );
